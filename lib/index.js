...
function _default(schema, opts) {
  ...
  schema.pre('findOneAndRemove', function (next) {
    ...
    this.model.findOne(this._conditions).session(session).then(original => deletePatches(original)).then(() => next()).catch(next);
  });
  schema.pre('findOneAndUpdate', preUpdateOne);
  function preUpdateOne(next) {
    const session = this.getOptions().session;
    this.model.findOne(this._conditions).session(session).then(original => {
      if (original) {
  ...
  function postUpdateOne(result, next) {
    ...
    const session = this.getOptions().session;
    this.model.findOne(conditions).session(session).then(doc => {
      if (!doc) {
        return;
      }
  ...
  function preUpdateMany(next) {
    const session = this.getOptions().session;
    this.model.find(this._conditions).session(session).then(originals => {
      const originalIds = [];
      const originalData = [];
  ...
  function postUpdateMany(result, next) {
    ...
    const session = this.getOptions().session;
    this.model.find(conditions).session(session).then(docs => Promise.all(docs.map((doc, i) => {
