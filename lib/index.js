...
const deepRemovePath = (patch, excludePath) => {
  ...
  if (isPathContained(operationPath, excludePath)) {
    let value = patch.value; // because the paths overlap start at patchPath.length
    ...
    if (typeof value[excludePath[excludePath.length - 1]] === 'undefined') return true;else {
      delete value[excludePath[excludePath.length - 1]]; // If the patch value has turned to {} return false so this patch can be filtered out
...
const sanitizeEmptyPath = path => path.length === 1 && path[0] === '' ? [] : path; // Checks if 'fractionPath' is contained in fullPath
...
const isIntegerGreaterEqual0 = entry => Number.isInteger(Number(entry)) && Number(entry) >= 0; // used to convert bson to json - especially ObjectID references need
// to be converted to hex strings so that the jsonpatch `compare` method
// works correctly


const toJSON = obj => JSON.parse(JSON.stringify(obj)); // helper function to merge query conditions after an update has happened
...
const mergeQueryConditionsWithUpdate = (_conditions, _update) => {
  ...
  const conditions = Object.assign({}, _conditions, update); // excluding updates other than $set
...
function _default(schema, opts) {
  const options = (0, _lodash.merge)({}, defaultOptions, opts); // get _id type from schema

  options._idType = schema.tree._id.type; // transform excludes option

  options.excludes = options.excludes.map(getArrayFromPath); // validate parameters
  ...
  (0, _assert.default)(options._idType, 'schema is missing an `_id` property'); // used to compare instance data snapshots. depopulates instance,
  ...
  schema.methods.data = function () {
    return this.toObject({
      ...
      transform: (doc, ret, options) => {
        delete ret._id; // if timestamps option is set on schema, ignore timestamp fields
  ...
  schema.methods.rollback = function (patchId, data, save = true) {
    return this.patches.find({
      ...
      date: 1
    }).exec().then(patches => new _bluebird.default((resolve, reject) => {
      // patch doesn't exist
      ...
      const apply = (0, _lodash.dropRightWhile)(patches, patch => patch.id !== patchId); // if the patches that are going to be applied are all existing patches,
      ...
      this.set((0, _lodash.merge)(data, state)); // in case of save, save it back to the db and resolve
  ...
  schema.virtual('patches').get(() => Patches); // after a document is initialized or saved, fresh snapshots of the
  ...
  schema.post('save', snapshot); // when a document is removed and `removePatches` is not set to false ,
  ...
  function deletePatches(document) {
    ...
    return document.patches.find({
      ref: document._id
    }).then(patches => (0, _bluebird.join)(patches.map(patch => patch.remove())));
  }
  ...
  schema.pre('remove', function (next) {
    ...
    deletePatches(this).then(() => next()).catch(next);
  ...
  schema.pre('save', function (next) {
    createPatch(this).then(() => next()).catch(next);
  });
  schema.pre('findOneAndRemove', function (next) {
    ...
    this.model.findOne(this._conditions).then(original => deletePatches(original)).then(() => next()).catch(next);
  });
  ...
  schema.post('findOneAndUpdate', function (doc, next) {
    ...
    createPatch(doc, this.options).then(() => next()).catch(next);
  });

  function postUpdateOne(result, next) {
    if (result.nModified === 0 && !result.upserted) return next();
    let conditions;
    if (this._originalId) conditions = {
      _id: {
        $eq: this._originalId
      }
    };else conditions = mergeQueryConditionsWithUpdate(this._conditions, this._update);
    this.model.findOne(conditions).then(doc => {
      if (!doc) return next();
      doc._original = this._original;
  ...
  function preUpdateMany(next) {
    this.model.find(this._conditions).then(originals => {
      const originalIds = [];
      const originalData = [];
  ...
  function postUpdateMany(result, next) {
    if (result.nModified === 0 && !result.upserted) return next();
    let conditions;
    if (this._originalIds.length === 0) conditions = mergeQueryConditionsWithUpdate(this._conditions, this._update);else conditions = {
      _id: {
        $in: this._originalIds
      }
    };
    this.model.find(conditions).then(docs => _bluebird.default.all(docs.map((doc, i) => {
